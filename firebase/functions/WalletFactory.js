...
const { PRECISION, CHAINS_RPC_LISTS } = require("./utils/c");
...
class WalletFactory {
    constructor(address, abi, rpcUrl, coin) {
        ...
        this.abi = abi;
        this.rpcUrl = rpcUrl;
        this.web3 = null//new Web3(rpcUrl);
        this.contract = null//new this.web3.eth.Contract(abi, address);
        this.coin = coin;
    }
    ...
    getSaltBalance(salt, blockNumber) {
        return new Promise(async (resolve, reject) => {
            try {
                const [web3, result] = await callWeb3Func(this.coin, this.web3, web3 => {

                    return new Promise((res, rej) => {
                        const contract = new web3.eth.Contract(this.abi, this.address)

                        const paddedSalt = web3.utils.rightPad(
                            web3.utils.asciiToHex(salt.toLowerCase()),
                            64
                        );
