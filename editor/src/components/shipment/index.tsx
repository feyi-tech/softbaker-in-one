...
import { 
  ...
  FaSignInAlt, FaSun, FaToolbox, FaTools, FaUndo, FaVideo, FaTelegramPlane, 
...
import { DEFAULT_DEPOSIT_AMOUNT, TEMP_TOOL_FORM_ID, Tool, URL_BASE, USE_FRONTBACKED_CONFIG } from '../../app-config';
...
import { useFrontbacked, useTools } from 'use-frontbacked'
import useColorValue from '../../hooks/useColorValue';
import useAdminInfoPermission from '../../hooks/useAdminInfoPermission';
...
const menu: MenuItem[] = [
  ...
  {
    id: MENU_ITEM_ID.toolEditor,
    title: "Tool Editor",
    icon: <FaPaintBrush />,
    showAlways: false
  },
  {
    id: MENU_ITEM_ID.syncUserBalance,
    title: "Sync User Balance",
    icon: <FaSync />,
    showAlways: false
  }
]
...
const DataView: React.FC<DataView> = ({ 
  ...
  const { 
    auth, user, authLoading, signIn, signOut, deposit,
    contact_link, group_link
  } = useFrontbacked();

  const { showTools, showTutorial } = useToolsSelector()
  const { getTool } = useTools(USE_FRONTBACKED_CONFIG)

  const adminPermissions = useAdminInfoPermission();
  ...
  const handleMenuItemClicked = (menuId: number) => {
    if(menuId == MENU_ITEM_ID.toolEditor) {
  ...
  const onMenuOptionState = (id: number): "show" | "hide" | "disable" | "loading" => {
    ...
    if(id == MENU_ITEM_ID.toolEditor && !adminPermissions.isToolsEditor) {
      return "hide"
    }
    if(id == MENU_ITEM_ID.syncUserBalance && !adminPermissions.isToolsEditor) {
      return "hide"
    }
  ...
  return (
    <>
      <Messenger 
        ...
        subTitle={
          !user? null : 
          <Text as="div" p="0px !important" m="0px !important" fontWeight="bold">
            { user.email }
          </Text>
        }
