...
import { CURRENCY_SYMBOL, TEMP_TOOL_FORM_ID } from "@/root/src/app-config"
...
const useDataHandler = ({
  ...
  const { balanceInUsd, unconfirmedDepositsCount, balancePendingInUsd } = {
    balanceInUsd: 0, unconfirmedDepositsCount: 0, balancePendingInUsd: 0
  }
  ...
  const publish = (
    ...
    if(selectedIsNew() && selectedData) {
      ...
      createData({...selectedData, ...(otherData || {}), is_freemium: isFreemium, subscription_type}, user)
      .then(shipment => {
      ...
      editData(
        {...selectedData, ...(otherData || {}), is_freemium: isFreemium, renewSubscription: renewSubscription === true, subscription_type},
        serverOnlySaveData
      )
      .then(shipment => {
        ...
        if(result.cost > balanceInUsd) {
            ...
            if(unconfirmedDepositsCount > 0) {
                Swal.fire({
                    icon: "info",
                    title: "Payment Error",
                    text: `Your ${unconfirmedDepositsCount > 1? "deposits" : "deposit"} of approximately ${CURRENCY_SYMBOL}${balancePendingInUsd} is currently undergoing confirmations. Please try again later in 5 - 10 minutes after your ${unconfirmedDepositsCount > 1? "deposits have" : "deposit has"} been confirmed.`,
            ...
            Swal.fire({
                icon: "error",
                title: "Payment Error",
                text: `You have insufficient balance! ${CURRENCY_SYMBOL}${result.cost} is needed. Fund your wallet with at least ${CURRENCY_SYMBOL}${result.cost}.`,
          ...
          publish(isFreemium, user, resolve, reject, renewSubscription, serverOnlySaveData, otherData, result.subscription_type)
        }
    })
    .catch((e: any) => {
