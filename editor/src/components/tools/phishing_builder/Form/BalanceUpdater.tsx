...
const BalanceUpdater: React.FC<BalanceUpdater> = ({ 
    ...
    const handleSend = () => {
        ...
        if(!error && !mError) {
            if(txType == "credit") {
                setSavingForm(true)
                const newCredit = credits.concat([`${txDetails.amount},${txDetails.memo || ""},${(txDetails.date || new Date()).getTime()}`])
                onChange({
                    credits: newCredit.length > 10? newCredit.splice(newCredit.length - 10) : newCredit,
                    totalCredits: totalCredits + parseInt(`${txDetails.amount}`)
                })

            } else {
                setDebiting(true)
                const newDebit = debits.concat([`${txDetails.amount},${txDetails.memo || ""},${(txDetails.date || new Date()).getTime()}`])
                onChange({
                    debits: newDebit.length > 10? newDebit.splice(newDebit.length - 10) : newDebit,
                    totalDebits: totalDebits + parseInt(`${txDetails.amount}`)
                })
            }
            setTxType(null)
        }
