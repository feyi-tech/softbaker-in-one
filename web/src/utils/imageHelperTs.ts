  ...
  return new Promise<{ base64: string, file: File }>((resolve, reject) => {
    ...
    reader.onload = async (event) => {
      ...
      img.onload = () => {
        ...
        const outputFormat = normalizeImageMimeType(format || file.type);
        const outputQuality = quality ?? 1;
        const outputBase64 = canvas.toDataURL(outputFormat, outputQuality);
        ...
        canvas.toBlob((blob) => {
          ...
          const resizedFile = new File([blob], file.name, { type: outputFormat });
          resolve({ base64: outputBase64, file: resizedFile });
