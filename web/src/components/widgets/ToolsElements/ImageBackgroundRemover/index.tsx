...
const CHECKING_TRANSPARENCY_MESSAGE = "Checking image transparency. Please be patient...";
const REMOVING_BACKGROUND_MESSAGE = "Removing background. Please be patient...";
const RETURNING_IMAGE_MESSAGE = "Preparing your image. Please be patient...";
const SKIPPING_REMOVAL_MESSAGE = "Saving your image. Please be patient...";
...
const ImageBackgroundRemover: React.FC<ImageBackgroundRemover> = ({ image, imageFile, onThumb, onImage, debug }) => {
    ...
    const [processingMessage, setProcessingMessage] = useState<string>(CHECKING_TRANSPARENCY_MESSAGE);

    const { startBackgroundRemoval, preloadBackgroundRemoval, progress, progressMessage } = useBgRemover(image, imageFile, debug);

    const returnImage = async (imageBase64: string, imageFile?: File) => {
        let thumb;
        let imgFile;
        if(imageFile) imgFile = base64UrlToFile(imageBase64, imageFile.name)
    ...
    useEffect(() => {
        if (image) {
            setProcessingMessage(CHECKING_TRANSPARENCY_MESSAGE);
            setProcessing(true);
            ...
            .then(async isTransparent => {
                ...
                if (isTransparent) {
                    setProcessingMessage(RETURNING_IMAGE_MESSAGE);
                    ...
                    .catch((e: any) => {
                        logger(`ImageBackgroundRemover.preloadBackgroundRemoval.error:${e?.message}`, LOGGER_LOG_TYPES.error)
                    })
                    setProcessing(false);
                }
                    ...
                    processing? (
                        ...
                        <Flex w="100%" 
                            ...
                            <AppButton fontSize="0.7rem" bgColor="tomato"
                                ...
                                onClick={() => {
                                    setProcessingMessage(REMOVING_BACKGROUND_MESSAGE);
                                    setProcessing(true);
                                ...
                                onClick={() => {
                                    ...
                                    .then(async (result) => {
                                        if(result.isConfirmed) {
                                            setProcessingMessage(SKIPPING_REMOVAL_MESSAGE);
                                            setProcessing(true);
                                            await returnImage(image, imageFile)
                                            setProcessing(false);
                                        }
