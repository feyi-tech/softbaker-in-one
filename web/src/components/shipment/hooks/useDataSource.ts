...
import { 
    collection, deleteDoc, doc, Firestore, limit, onSnapshot, orderBy, query, where 
...
import { base64ToFile, uploadWithRetry } from "../../tools/tool_viewer_data_stream/core/svg-processor/utils";
...
const useDataSource = (
  ...
  const { user, authLoading, db, auth, createDoc, updateDoc, uploadFile } = useSoftBaker()
  ...
  useEffect(() => {
    ...
    const uid = user.uid //"8kecn7EL4eQvCwvSV7pngb9Oih83" //  
  ...
  const editData = (updatedData: Message, serverOnlySaveData?: Message | null): Promise<Message> => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        for (const field of updatableFields) {
            ...
            if(previousUpdatedData[field] != data[field]) update[field] = data[field]
        }
