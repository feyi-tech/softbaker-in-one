...
const parseBody = (body: string, width: number, height: number, maxChars: number) => {
    ...
    return lines.map((line, index) => {
        if (line === '') return '';
        const dy = responsiveDY(lines[index - 1] === '' ? 84.7931 : 55.567, height);
        return `<tspan x="0" ${index === 0 ? 'y="0"' : `dy="${dy}"`}>${escapeHtmlEntities(line)}</tspan>`;
    }).join('');
    ...
    return lines.map((line, index) => {
      return line === ''
        ? ''
        : `<tspan x="0" dy="${index === 0 ? 0 : responsiveDY(lines[index - 1] === '' ? 84.7931 : 55.567, height)}">${escapeHtmlEntities(line)}</tspan>`;
    }).join('');*/
};
...
const getSvg = ({ 
    ...
    const bodyLines: string[] = formatText(body || "", BODY_MAX_CHARS)
    const bodyHeight = bodyLines.length * BODY_LINE_HEIGHT;
    const signatoryY = bodyY + bodyHeight + (BODY_LINE_HEIGHT * 2); //bodyY + BODY_MARGIN_BOTTOM

    return `<svg id="PAPER" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
    width="${width}" height="${height}" viewBox="0 0 1024 1448">
    <defs>
      <style>
    ...
    <text id="SAMPLE_COMPAY_NAME" data-name="SAMPLE COMPAY NAME" class="cls-3" transform="${translate(511.839, 1403.89, width, BASE_TEMPLATE_WIDTH, height, BASE_TEMPLATE_HEIGHT)} ${scale(0.413, 0.413, width, BASE_TEMPLATE_WIDTH, height, BASE_TEMPLATE_HEIGHT)}">
        <tspan x="${getResponsiveX(0, width, BASE_TEMPLATE_WIDTH, FRACTION_DIGITS)}">${escapeHtmlEntities(companyName)}</tspan>
    </text>
    ${
        ...
        ? `<text id="senderInfo" data-name="senderInfo" class="cls-14" transform="${translate(
            ...
            ${senderInfo.split(NEW_LINE).map((info: string, index: number) => {
                return `<tspan id="sender_info_${index}" x="${getResponsiveX(
                    ...
                    BASE_TEMPLATE_WIDTH,
                    FRACTION_DIGITS
                )}" dy="${index == 0 ? '0' : responsiveDY(73.249, height)}">${escapeHtmlEntities(info)}</tspan>`;
            }).join('')}
        </text>`
        : ''
    }
        ...
        ? `<text id="recipientInfo" data-name="recipientInfo" class="cls-10" transform="${translate(
            ...
            ${recipientInfo.split(NEW_LINE).map((info: string, index: number) => {
                return `<tspan id="recipient_info_${index}" class="cls-11" x="${getResponsiveX(
                    ...
                    BASE_TEMPLATE_WIDTH,
                    FRACTION_DIGITS
                )}" dy="${index == 0 ? '0' : responsiveDY(60.567, height)}">${escapeHtmlEntities(info)}</tspan>`;
            }).join('')}
        </text>`
        : ''
    }
    <text id="TITTLE" class="cls-9" 
        ...
        <tspan x="${getResponsiveX(0, width, BASE_TEMPLATE_WIDTH, FRACTION_DIGITS)}">${escapeHtmlEntities(title)}</tspan>
    </text>
    <text id="BODY-2" data-name="BODY" fill="#030303" class="cls-8" 
    transform="${translate(70.274, bodyY/*629.243*/, width, BASE_TEMPLATE_WIDTH, height, BASE_TEMPLATE_HEIGHT)} ${scale(0.413, 0.413, width, BASE_TEMPLATE_WIDTH, height, BASE_TEMPLATE_HEIGHT)}">
        ${body? parseBody(body, width, height, BODY_MAX_CHARS) : ""}
    </text>
    ${
        ...
        `${signatories.map((signatory, index) => {
            ...
            <g id="signatory_${index}">
                ...
                transform="${translate(70.606, signatoryY + 100.081/*1104.081*/ + (index * SIGNATORY_MARGIN_BOTTOM), width, BASE_TEMPLATE_WIDTH, height, BASE_TEMPLATE_HEIGHT)} ${scale(0.413, 0.412, width, BASE_TEMPLATE_WIDTH, height, BASE_TEMPLATE_HEIGHT)}">
                    ${signatory.name && signatory.name.length > 0? escapeHtmlEntities(signatory.name) : ""}${signatory.title && signatory.title.length > 0 ? signatory.name && signatory.name.length > 0? `(${escapeHtmlEntities(signatory.title)})` : escapeHtmlEntities(signatory.title) : ''}
                </text>
            </g>`;
        }).join('')}`
        : ''
