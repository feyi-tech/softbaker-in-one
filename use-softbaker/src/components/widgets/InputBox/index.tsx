...
const updateRangeValue = useCallback((min: any, max: any) => {
  if(!isNaN(min)) {
...
const memoizedOptions = useMemo(() => {
  // Compute and memoize the options array here
  return (options || []).map((v, i) => {
    if (onOptionValue && onOptionName) {
      return <option key={i} value={onOptionValue(v, i)}>{onOptionName(v, i)}</option>;
    } else {
      return null;
    }
  });
}, [options, onOptionValue, onOptionName]);
  ...
  <FormControl isInvalid={!nullOrEmpty(errorMessage) && type != TYPES.location} 
    ...
    }
    {
      ...
      <InputGroup w="100%" h="100%">
        ...
        <Textarea id={id} key={key} onFocus={onFocus} disabled={disabled} ref={inputRef} w="100%" h="100%" placeholder={placeholder as string} isDisabled={disabled} 
      ...
      <Flex w="100%" flexDirection="row" flexWrap="wrap">
      {
        checklist.map((check, index) => (
          <Checkbox id={id? `${id}_${index}` : `${index}`} key={key? `${key}_${index}` : index} onFocus={onFocus} disabled={disabled} ref={inputRef} 
