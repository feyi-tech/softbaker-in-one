...
const { COINS, PRICE_DATA_TTL_MINUTES, REF_PCT, MAX_BIG_INT_STRING_DIGITS, MAX_RISKY_OPERATION_LOGIN_AGE_IN_SECONDS, DOLLAR_MARKET_PRICE_SHIFT_FILL } = require('./utils/c');
...
const updateBalance = (uid, coins, referralId, userEmail) => {
  return new Promise(async (resolve, reject) => {
    if(await walletUpdatePaused()) return reject({ error: "Balance update currently disabled. Please try again later" });
    ...
    .then(async (balanceDocSnapshot) => {
      ...
      } if(referralId && referralId != uid && await userExists(referralId)/*Check if the referral id exists as a user.*/) {
        refId = referralId;
      }
        ...
        promises.push(new Promise(async (resolve, reject) => {
          ...
          try {
            ...
            if (confirmationsInfo.error) {
              ...
              var discountBalanceDiff = priceDiffInUsd

              var bonus = calculateHighDepositBonus(discountBalanceDiff + DOLLAR_MARKET_PRICE_SHIFT_FILL)
              if(refId) {
                //Adds the same percentage to the referred user's deposit
              ...
              const walletUpdateData = {
                ...
                [getCoinBalanceKey(coin, "balance_in_usd")]: FieldValue.increment(discountBalanceDiff),
                [getCoinBalanceKey(coin, "contract_created")]: confirmationsInfo.walletCreated,
                usd_balance: FieldValue.increment(discountBalanceDiff),
                deposit_counts: FieldValue.increment(1),
                last_updated: FieldValue.serverTimestamp()
              }

              var depositAmountInSatoshi = BigInt(confirmationsInfo.confirmedDepositsBalance - readServerCoinBalance(coin, balanceDoc));

              var referralEarnings = 0;
              var siteEarnings = depositAmountInSatoshi

              var statsData = {
                [getCoinBalanceKey(coin, "users_balance_in_coin")]: FieldValue.increment(balanceDiffInCoin),
                [getCoinBalanceKey(coin, "users_usd_balance")]: FieldValue.increment(priceDiffInUsd),
                users_usd_balance: FieldValue.increment(priceDiffInUsd),
                [getCoinBalanceKey(coin, "users_usd_referral_discount_balance")]: FieldValue.increment(discountBalanceDiff),
                users_usd_referral_discount_balance: FieldValue.increment(discountBalanceDiff),
                users_deposit_counts: FieldValue.increment(1),
                update_counts: FieldValue.increment(1),
                last_updated: FieldValue.serverTimestamp()
              }

              if(refId) {
                referralEarnings = (depositAmountInSatoshi * BigInt(REF_PCT)) / BigInt(100)
                ...
                const refWalletUpdateData = {
                  ...bigIntIncrements,
                  [getCoinBalanceKey(coin, "referral_earnings_counts")]: FieldValue.increment(1),
                  referral_earnings_counts: FieldValue.increment(1),
                  last_updated: FieldValue.serverTimestamp()
                }
                //If this is the first time the user is making payment
                if(!balanceDocSnapshot.exists) {
                  //log the one-time-first-time referral info on this payment
                  refWalletUpdateData.total_referrals = FieldValue.increment(1);
              ...
              if(!balanceDocSnapshot.exists) {
                statsData[refId? "total_paying_referred_users" : "total_paying_direct_users"] = FieldValue.increment(1)
                statsData["total_paying_users"] = FieldValue.increment(1)
...
const ruleDocToRule = (ruleDoc, doc, isEdit, uid, dynamicToolId) => {
  ...
  if(dynamicToolId) {
    //Create an x for it.
    const x = {}
    //Only is_freemium is editable
    const updatableFields = ["is_freemium"]
...
const origins = [
  ...
  'http://localhost:4000',  
  'http://localhost:5000',  
  'http://mycargoroute.local',   
  'https://mycargoroute.com', 
  'https://softbaker.com',
  'http://bank.local',   
  'https://banknordica.ru', 
  'http://flight.local',  
  'https://myskyroute.com', 
...
const validateRecaptcha = async (req, res, next) => {
  ...
  const body = `secret=${process.env.G_RECAPTCHA_SECRET}&response=${response}`
  ...
  .then(verification => {
    if(verification.data.success === true) {
      ...
      res.status(401).json({ 
        ...
        //+ JSON.stringify(verification.data) + (process.env.G_RECAPTCHA_SECRET || "NONE") 
      });
    }
...
app.post('/update_balance', authenticateFirebaseUser, async (req, res) => {
  const { uid } = req;
  const { coins, referralId, userEmail } = req.body;
  ...
  updateBalance(uid, coins, referralId, userEmail)
  .then(result => {
...
const canUpload = async (uid, dir, db) => {
  // Prefixes
  ...
  if (folder.startsWith(`/users/${uid}`)) {
    return true;
  }
...
app.post('/get-presigned-url', authenticateFirebaseUser, async (req, res) => {
  ...
  const canUploadFile = await canUpload(uid, dir)
  if(!canUploadFile) {
...
app.post('/get-presigned-url-2', async (req, res) => {
  const token = req.headers.authorization;

  if (!token) {
    return res.status(401).json({ error: 'Unauthorized: No token provided' });

  } else if (token !== "Is_Long_Eyes") {
...
app.get('/storeframe', async (req, res) => {
  // Set the Content-Type header to text/html
  res.header('Content-Type', 'text/html');
  
  // Send the HTML file as the response
  res.sendFile(path.join(__dirname, 'storeframe.html'));
});
